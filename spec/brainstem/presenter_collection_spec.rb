  ...
  describe "#presenting" do
    ...
    describe "filters" do
      ...
      it "limits records to those matching given filters" do
        ...
        expect(result[:workspaces].keys.all? {|id| bob_workspaces_ids.map(&:to_s).include?(id) }).to be_true # all of the returned workspaces should contain bob
      end
      ...
      it "returns all records if filters are not given" do
        ...
        expect(result[:workspaces].keys.all? {|id| bob_workspaces_ids.map(&:to_s).include?(id) }).to be_false # the returned workspaces no longer all contain bob
      end
      ...
      it "ignores unknown filters" do
        ...
        expect(result[:workspaces].keys.all? {|id| bob_workspaces_ids.map(&:to_s).include?(id) }).to be_false
      end
      ...
      it "ensures arguments are strings" do
        WorkspacePresenter.filter(:owned_by_bob) { |scope, string| expect(string).to be_a(String); scope }
