      ...
      if using_mysql
        ...
        context 'when using mysql' do
          ...
          it 'uses the mocked paginator and only queries the database once' do
            mock(paginator).get_ids_for_page(page, anything) { fake_get_ids_for_page_results }
            mock(paginator).get_count(anything) { fake_get_count_results }

            expect { strategy.evaluate_scope(Workspace.unscoped) }.to make_database_queries(count: 1)
            expect { strategy.evaluate_count(Workspace.unscoped) }.to_not make_database_queries
          end
          ...
          context 'when custom paginator fails' do
            ...
            it '#evaluate_count gets count from paginator' do
              mock(paginator).get_count(anything) { 345 }

              expect(strategy.evaluate_count(Workspace.unscoped)).to eq 345
            end
          end
        end
      else
